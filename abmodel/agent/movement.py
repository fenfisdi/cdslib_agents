    ...
    def update_velocities(cls, df: DataFrame, distribution: Distribution,
        ...
        def avoid_agents(cls, df: DataFrame, df_to_avoid: DataFrame) -> None:
            ...
            def deviation_angle(grouped_serie: Series) -> float:
                """
                """
                sorted_serie = grouped_serie["relative_angle"].sort_values().copy()

                consecutive_angle = sorted_serie.diff().shift(periods=-1)
                consecutive_angle.iloc[-1] = 2*pi + (sorted_serie.iloc[-1] -
                        sorted_serie.iloc[0])
            ...
            scary_agents["relative_angle"] = cls.vector_angles(scary_agents,
            ...
            new_angles = scary_agents[["agent",
                "relative_angle"]].groupby("agent").apply(
                        deviation_angle
                        )
