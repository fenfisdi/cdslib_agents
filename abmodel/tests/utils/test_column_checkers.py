...
class TestCaseFieldExistenceAndErrors():
    """
        Verify the functionality of the methods check_field_existance() and
        check_field_errors() from utilities, using unitary test with the Python
        testing tool pytest.
    """
    ...
    def fixture_field_errors(self, scope = 'class') -> None:
        ...
        pytest.expected_string_debug_False = ("Some columns might be initialized incorrectly.\n"
                    "To pinpoint especific errors, "
                    "add `debug=True` as a parameter")

    def test_field_existence_cols_in(self, fixture_field_existence):
        """List passed containing columns beloging the DataFrame."""
    ...
    def test_field_existence_cols_is_not_in(self, fixture_field_existence):
        """List passed containing columns do not beloging the DataFrame."""
    ...
    def test_field_existence_output_string(self, fixture_field_existence):
        """Verify whether the ouput string corresponds with the columns input list is not in the DataFrame."""

        with pytest.raises(ValueError, match=pytest.expected_ouput_cols_is_not_in):
            assert check_field_existance(pytest.df, pytest.cols_is_not_in)

    def test_field_errors_na_values_debug_True(self, fixture_field_errors):
        """Verify whether the ouput ValueError string corresponds the expected for Null values; debug True."""
    ...
    def test_field_error_na_values_debug_False(self, fixture_field_errors):
        """Verify whether the ouput ValueError string corresponds the expected for Null values; debug False."""
