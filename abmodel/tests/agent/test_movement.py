...
class TestCaseAgentMovement:
    ...
    @pytest.fixture
    def fixture_crash_with_wall(self, set_up, scope='method') -> None:
        pytest.data = {
            'x': [49, 0, -49, 0],
            'y': [0, 29, 0, -29],
            'vx': [2.0, 2.0, -2.0, 2.0],
    ...
    @pytest.fixture
    def fixture_crash_with_corner(self, set_up, scope='method') -> None:
        pytest.data = {
            'x': [49, -49, -49, 49],
            'y': [29, 29, -29, -29],
            'vx': [2.0, -2.0, -2.0, 2.0],
    ...
    def test_movement_function_field_error(self, set_up):
    ...
    def test_vector_angles_raise_error(self,
                                       fixture_test_vector_angles_raise_error):
        """
        Raises an exception when the input DataFrame columns 'vx' and 'vy'
        does not exist.
        """
    ...
    def test_update_velocities_angle_variance_0(self,
                                                fixture_update_velocities):
        """
        Verifies wheter there are no changes in the velocities components when
        the standard deviation of the normal distribution is set to zero
        (angle_variance=0.0).
        """
        df = DataFrame(pytest.data)
    ...
    def test_update_velocities_comparation_normal_distribution(
                                            self, fixture_update_velocities):
        """
        Comparison of the velocities magnitude of agents with a given
        distribution (normal), through a Kolmogorov-smirnov test.
        """
        df = DataFrame(pytest.data)
        df = AgentMovement.update_velocities(df, pytest.distrib, 0.0)
    ...
    def test_update_velocities_3(self, fixture_update_velocities):
        """
        Comparison of the angles of the agents with a given distribution
        (normal), through a Kolmogorov-smirnov test.
        """
        df = DataFrame(pytest.data)
        angles_before = AgentMovement.vector_angles(df, ['vx', 'vy'])
    ...
    def test_avoid_agents(self, fixture_avoid_agents):
        """Avoids one agent of a DataFrame."""
